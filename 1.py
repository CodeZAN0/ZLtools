...
class DomainInfoManager:
    def __init__(self, root):
        ...
        self.labelpf = tk.Label(self.frameff, text="Password.txt:", font=("Helvetica", 12, 'bold'), justify='center',
                               fg="green", bg='#0e1116')
        self.labelpf.pack()
    ...
    def create_section_frame(self, section_name):
                ...
                def save_results_to_file(results_dict):
                    ...
                    try:
                        ...
                        for url in input_urls:
                            ...
                            url_folder_name = os.path.join(folder_name, f"{cleaned_url}_{formatted_date}")
        ...
        #Proxy
            ...
            # Example placeholder functions for Discord Tokens and User Info extraction
            def extract_discord_tokens(folder_path):
                return [{"Token": "example_token"}]

            def extract_user_info(folder_path):
            ...
            def extract_info():
                ...
                if credit_card_info:
                    ...
                    for info in credit_card_info:
                        result_text.insert(tk.END, "CardType: {} | ".format(info.get("CardType", "")))
                        result_text.insert(tk.END, "Holder: {} | ".format(info.get("Holder", "")))
                        result_text.insert(tk.END, "Card: {} | ".format(info.get("Card", "")))
                        result_text.insert(tk.END, "Expire: {}\n".format(info.get("Expire", "")))
            ...
            # Input frame
            frame_input = ttk.Frame(frame_cen, style='Main.TFrame')
            frame_input.pack(pady=20)

            folder_label = tk.Label(frame_input, text="Select Folder:", font=("Helvetica", 14, 'bold'), foreground='red', background='#0e1116')
            folder_label.pack(padx=5, pady=5, side=tk.LEFT)

            folder_path_entry = tk.Entry(frame_input, width=50, font=("Helvetica", 12, 'bold'))
            folder_path_entry.pack(padx=5, pady=5, side=tk.LEFT)

            # Button frame
            frame_but = ttk.Frame(frame_input, style='Main.TFrame')
            frame_but.pack(pady=20, side=tk.LEFT)

            browse_button = tk.Button(frame_but, text="   Browse   ", height=2, font=("Helvetica", 12, 'bold'), command=browse_folder)
            browse_button.pack(padx=5, pady=5)

            extract_button = tk.Button(frame_but, text="Extract Info", height=2, font=("Helvetica", 12, 'bold'), command=extract_info)
            extract_button.pack(padx=10, pady=5)

            # Result text
            result_text = tk.Text(main_frame, height=20, width=80, font=("Helvetica", 14, "bold"), wrap="word", fg="green", bg="white")
            result_text.pack(padx=5, pady=5, side=tk.BOTTOM)


            tab2frame = ttk.Frame(tab2,)
            tab2frame.pack()
...
class LoginApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Login ZL Tools")
        self.root.geometry("400x450+700+300")
        self.root.resizable(False, False)
        self.root.configure(background='#222222')
        self.setup_styles()
        # Create logo frame at the top
        self.logo_frame = ttk.Frame(self.root, padding=10,style='Main.TFrame')
        self.logo_frame.pack(side=tk.TOP, fill=tk.BOTH)

        # Load and display logo (replace 'logo.png' with your logo path)
        self.image = tk.PhotoImage(file='logo.png')
        self.logo_label = ttk.Label(self.logo_frame, image=self.image,background='#0e1116')
        self.logo_label.pack()

        self.tt = ttk.Label(self.logo_frame, text=f"ZL TOOLS {version}",font=('Helvetica' ,14 ,'bold'),background='#0e1116',foreground='Yellow')
        self.tt.pack()

        # Create content frame between logo and buttons
        self.content_frame = ttk.Frame(self.root, padding=10,style='Main.TFrame')
        self.content_frame.pack(side=tk.TOP, fill=tk.BOTH, expand=True)

        # Create frames for different login options
        self.key_frame = self.create_key_login_frame()
        self.username_frame = self.create_username_login_frame()

        # Show the key login frame by default
        self.show_frame(self.key_frame)

        # Create navigation buttons at the bottom
        self.button_frame = ttk.Frame(self.root, padding=10,style='Main.TFrame')
        self.button_frame.pack(side=tk.BOTTOM, fill=tk.X)

        self.key_button = ttk.Button(self.button_frame, text="Login with Key", command=self.show_key_frame)
        self.key_button.pack(side=tk.LEFT, padx=10, pady=5)

        self.username_button = ttk.Button(self.button_frame, text="Login with Username", command=self.show_username_frame)
        self.username_button.pack(side=tk.LEFT, padx=10, pady=5)
        ...
        self.db_connection = mysql.connector.connect(
            ...
            user="if0_36642928",  # Update with your MySQL username
            password="zanko@1447",  # Update with your MySQL password
            database="if0_36642928_mywebsite"  # Update with your MySQL database name
        )

    def create_key_login_frame(self):
        frame = ttk.Frame(self.content_frame, padding=10,style='Main.TFrame')
        label = ttk.Label(frame, text="Enter Your Key", background='#0e1116', foreground='white', font=("Arial", 15))
        label.pack(pady=20)

        self.key_entry = ttk.Entry(frame, width=30)
        self.key_entry.pack(pady=10)

        key_button = ttk.Button(frame, text="Login", command=self.login_with_key)
        key_button.pack(pady=10)

        return frame

    def create_username_login_frame(self):
        frame = ttk.Frame(self.content_frame, padding=10,style='Main.TFrame')
        label = ttk.Label(frame, text="Enter Username and Password", background='#0e1116', foreground='white', font=("Arial", 15))
        label.pack(pady=20)
        ...
        login_button = ttk.Button(frame, text="Login", command=self.login_with_username)
    ...
    def show_frame(self, frame):
        # Hide all content frames and then show the specified frame
    ...
    def show_username_frame(self):
        self.show_frame(self.username_frame)

    def login_with_key(self):
        user_key = self.key_entry.get()
        try:
            cursor = self.db_connection.cursor()
            query = "SELECT * FROM user_keys WHERE generated_key = %s"
            cursor.execute(query, (user_key,))
            result = cursor.fetchone()
            cursor.close()

            if result:
                messagebox.showinfo("Success", "Login Successful with Key!")
                self.hide_login()  # Hide the LoginApp window
                self.open()  # Open the DomainInfoManager window

            else:
                messagebox.showwarning("Warning", "Invalid Key. Please try again.")
    ...
    def login_with_username(self):
        ...
        try:
            ...
            if result:
                messagebox.showinfo("Success", "Login Successful with Username/Password!")
                self.hide_login()  # Hide the LoginApp window
                self.open_domain_info_manager()  # Open the DomainInfoManager window
            else:
                messagebox.showwarning("Warning", "Invalid Username or Password. Please try again.")

        except mysql.connector.Error as e:
            messagebox.showerror("Database Error", f"Error: {e}")

    def hide_login(self):
        self.root.withdraw()  # Hide the LoginApp window

    def open(self):
        top_level = tk.Toplevel(self.root)
        app = DomainInfoManager(top_level)
